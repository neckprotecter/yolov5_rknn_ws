...
import cv2
import numpy as np
from yolov5_rknn_ros.py_utils.coco_utils import COCO_test_helper
...
class yoloV5RKNN:
    def __init__(self, model_path, anchors_path, target='rk3566', device_id=None, obj_thresh=OBJ_THRESH, nms_thresh=NMS_THRESH):
        ...
        if ext == 'pt' or ext == 'torchscript':
            from .py_utils.pytorch_executor import Torch_model_container
            self.model = Torch_model_container(model_path)
            ...
            self.model = ONNX_model_container(model_path)
            self.platform = 'onnx'
        else:
    ...
    def preprocess(self, img):
        img = self.co_helper.letter_box(im=img, new_shape=(IMG_SIZE[1], IMG_SIZE[0]), pad_color=(0, 0, 0))
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        if self.platform in ['pytorch', 'onnx']:
            input_data = img.transpose((2, 0, 1))
            input_data = input_data.reshape(1, *input_data.shape).astype(np.float32) / 255.
        else:
            input_data = img
        return input_data
    ...
    def detect(self, img):
        ...
        outputs = self.model.run([np.expand_dims(input_data, 0)])
    ...
    def draw(self, img, boxes, classes, scores):
        ...
        for box, cl, score in zip(boxes, classes, scores):
            ...
            cv2.putText(img, f"{label} {score:.2f}", (x1, y1 - 6),
...
# # py_utils_path = os.path.join(cur_dir, 'py_utils')
...
#     '''
#     # 替换原始的for图片循环 - camera
#     cap = cv2.VideoCapture(0)  # 打开摄像头（0是默认摄像头）
...
#             cv2.imshow('YOLOv5 RKNN Real-time', img_src)
